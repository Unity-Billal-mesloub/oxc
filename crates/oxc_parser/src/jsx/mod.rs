...
/// Result of parsing JSX children - either found the closing element or closing fragment
enum JSXClosing<'a> {
...
impl<'a> ParserImpl<'a> {
    ...
    /// `span` is the start span (pointing to `<`).
    fn parse_jsx_closing_inline(&mut self, span: u32, in_jsx_child: bool) -> JSXClosing<'a> {
        // Check if this is a closing fragment (`</>`) or closing element (`</name>`)
        if self.at(Kind::RAngle) {
            ...
            JSXClosing::Fragment(self.ast.jsx_closing_fragment(self.end_span(span)))
        } else {
            ...
            JSXClosing::Element(self.ast.alloc_jsx_closing_element(self.end_span(span), name))
        }
    }
